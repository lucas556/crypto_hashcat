// sha512_host.c
// 对输入文件每一行做 SHA512（不含行尾 '\n'），输出 64 字节摘要的 hex。
//
// 使用 hashcat 的 inc_* + sha512_wrapper.cl，支持分批处理巨量行。
...
// digest[LEN] -> hex[LEN*2+1]
static void digest_to_hex (const uint8_t *digest, size_t len, char *hex_out)
{
...
int main (int argc, char **argv)
{
  ...
  // 1. 选 OpenCL 平台 & 设备（和 sha256-host 版一样）
  ...
  // 3. 读取 & 编译 sha512_wrapper.cl
  size_t src_size = 0;
  char *src = read_text_file ("sha512_wrapper.cl", &src_size);
  ...
  const char *build_opts = NULL; // 如果 inc_* 不在当前目录，在这里加 -I 路径
  ...
  cl_kernel kernel = clCreateKernel (program, "sha512_wrapper", &err);
  ...
  for (;;)
  {
    ...
    // 注意：SHA512 digest = 16 个 u32 = 64 字节
    cl_mem buf_out = clCreateBuffer (
        context,
        CL_MEM_WRITE_ONLY,
        (size_t) num_msgs * 16u * sizeof (uint32_t),
        NULL,
        &err);
    ...
    uint32_t *digests_host =
        (uint32_t *) malloc ((size_t) num_msgs * 16u * sizeof (uint32_t));
    if (!digests_host)
    {
    ...
    char hex[128 + 1];     // 64 字节 → 128 hex
    uint8_t digest[64];

    for (uint32_t k = 0; k < num_msgs; k++)
    {
      uint32_t *d = digests_host + (size_t) k * 16u;

      // 将 16 个 u32 直接按 big-endian 展开成 64 字节：
      for (int j = 0; j < 16; j++)
      {
        uint32_t v = d[j];
      ...
      digest_to_hex (digest, 64, hex);
      fprintf (fout, "%s\n", hex);
    }

    // 首批打印第一行做 sanity check
    if (batch_index == 1 && num_msgs > 0)
    {
      uint32_t *d0 = digests_host + 0 * 16u;
      for (int j = 0; j < 16; j++)
      {
        uint32_t v = d0[j];
        digest[j*4 + 0] = (uint8_t)((v >> 24) & 0xff);
      ...
      char hex0[129];
      digest_to_hex (digest, 64, hex0);
      fprintf (stderr, "[OpenCL] First line SHA512 = %s\n", hex0);
    }
